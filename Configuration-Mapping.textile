...
Each method of the ConfigurationProvider takes a <code>ConfigurationKey</code> as an argument as opposed to a simple string. The ConfigurationKey enables a convention for provided variables within the @Configuration value. Governator will parse the value into tokens. Tokens can be either a literal value or a variable substitution. Variables are specified as <code>${variable-name}</code>. The ConfigurationKey has a method to apply variables and return the applied key:
